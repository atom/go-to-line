...
class GoToLineView {
  constructor () {
    ...
    this.miniEditor.onWillInsertText((event) => {
      const fullText = this.miniEditor.getText() + event.text
      const hasInvalidCharacter = event.text.match(/[^0-9:]/)
      const hasMultipleColons =
        (fullText.match(/:/g) || []).length > 1

      if (hasInvalidCharacter || hasMultipleColons) {
        event.cancel()
      }
    })
